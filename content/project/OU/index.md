...
Make a new column - if the difference between dates is more than that), the submission was late.
Aggregate by student ID, module, and module presentation. |
| 4 | Merge dataframes | VLE + VLE materials = total_click_per_student
We can merge these two tables with an inner merge as resources with no activity for any student to provide zero information. We will drop week_from, week_to, and date columns.
...
| 5 | Missing values | IMD band: 
Fill them according to the most frequent band for that region.
Date registration: 
For the withdrawn students, we will subtract the median value from the registration date to fill these.
total_click:
We will  replace them with 0s (meaning not interested in it).
weighted_score:
We will  replace the nan values with 0s (meaning do not make submissions).
late_rate:
We will replace the nan values with 1.00 (100% late rate).
fail_rate:
